...
fn test_graph_with_one_edge() {
    ...
    // Iirii: According to Graphviz, there should be two nodes:
    // https://dreampuf.github.io/GraphvizOnline/#graph{a--b}
    // i.e. `assert_eq!(graph.nodes.len(), 2);`
    // So, the following assertion is false:
    assert!(graph.nodes.is_empty());
