...
#[test]
#[ignore]
fn test_add_minutes_mut() {
    let mut clock = Clock::new(10, 0);
    clock.add_minutes_mut(3);
    assert_eq!(clock.to_string(), "10:03");
}
...
#[test]
#[ignore]
fn test_time_keeping_mut() {
    let mut clock = Clock::new(10, 0);
    assert_eq!(clock.to_string(), "10:00");
    std::thread::sleep(std::time::Duration::from_secs(62));
    assert_eq!(clock.to_string(), "10:01");
    clock.add_minutes_mut(-1);
    assert_eq!(clock.to_string(), "10:00");
...
#[test]
#[ignore]
fn test_time_keeping() {
    let clock = Clock::new(10, 0);
    assert_eq!(clock.to_string(), "10:00");
    std::thread::sleep(std::time::Duration::from_secs(62));
    assert_eq!(clock.to_string(), "10:01");
    let clock = clock.add_minutes(-1);
    assert_eq!(clock.to_string(), "10:00");
    std::thread::sleep(std::time::Duration::from_secs(59));
    assert_eq!(clock.to_string(), "10:01");
}
